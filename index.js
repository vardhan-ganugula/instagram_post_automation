...
async function addText({ fileName, quote }) {
  ...
  const image = await loadImage("./images/" + fileName + ".png");
  ...
  ctx.fillText("- " + process.env.CREATOR_NAME, canvasSize * 0.8, canvasSize * 0.95);
...
const generatePostImage = async ({ prompt, quote }) => {
  const time = Date.now();
  try {
    await generateImage({
      prompt,
...
1. "motivationalLine": A short, powerful and unique motivational quote by any famous person based on the randomness (can include \\n for formatting).
...
const generatePost = async () => {
  ...
  await generatePostImage({ prompt: imagePrompt, quote: motivationalLine });
  await postToInsta({
